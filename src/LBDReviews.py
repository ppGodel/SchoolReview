...
def create_lbd_repo_calif(querier: Callable, base_csv_path: str) -> DataFrame:
...
def evaluate_lbd_course(practices_list: List[Practice], config_path: str, target_csv_path: str,
    ...
    def wrap_create_lbd_class(querier: Callable, csv_path: str) -> Callable[[str], DataFrame]:
        def create_lbd_course() -> DataFrame:
            return create_lbd_repo_calif(querier, csv_path)
        return create_lbd_course

    querier = get_querier_with_credentials(config_path)
    return evaluate_class(practices_list, wrap_create_lbd_class(querier, base_csv_path),
...
if __name__ == '__main__':
    ...
    reviewed_practices = evaluate_lbd_course(practices, credentials, target_csv, class_info_csv)
